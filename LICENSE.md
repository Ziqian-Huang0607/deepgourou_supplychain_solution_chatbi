Copyright (c) 2026 Ziqian-Huang0607 (Team Deepgourou)
===============================================================================
License Proprietary (Version 1.4), May 2026, P.R. China Edition, by willuhd.

This Repository and all contents within it contain code that is fully 
proprietary. They belong to the Repository's Owners. These files are NOT open 
source. **ALL RIGHTS RESERVED**. 

Definitions
-----------
- "Owners": Ziqian-Huang0607 (Team Deepgourou). The Owners are the sole copyright holders
  of this repository.
- "Team Deepgourou": Ziqian Huang, Ethan Cai.
- "Repository": this Git repository ("willuhd/HKU-AI-Challenge"), including but not
  limited to all code, media, Git commit history, and content within it.
- "Software": all source code, assets, documentation, and associated files.
- "You" (or "Licensee"): Any individual or legal entity exercising permissions 
  granted by this License. By viewing this code, you automatically accept this
  license and are subject to all liabilities imposed within it.

Ownership
---------
This Software, including all source code, assets, documentation, and
associated files, is the exclusive property of the Owners. 
No ownership rights are transferred to anyone else.

License Grant
-------------
No rights are granted to use, copy, modify, merge, publish, distribute, 
sublicense, sell, or otherwise exploit the Software unless express written 
permission is provided by the Owners.

Internal Use Restriction
------------------------
If written permission is granted, such permission applies only to internal 
development within the project. The Software may not be used in any other 
Repository, project, product, service, or environment without additional 
written authorization.

Prohibited Actions
------------------
Unless prior written permission is provided, you may not:
- Use the Software for any purpose.
- Copy or redistribute the Software in any form.
- Modify, adapt, translate, or create derivative works of the Software.
- Reverse-engineer or try to build a public rip-off competitor to the Software.
- Remove, alter, or obscure copyright notices, author credits, or attribution
  within the Software.
- Claim authorship, ownership, or contribution to the Software, including
  through superficial changes such as variable renaming, formatting adjustments,
  changing headers, or other minimal alterations that can be easily traced
  back to the original source.

Allowed Actions
---------------
You may request usage of the Software from its original Owner. Under Fair Use, 
you are allowed to read, reference, quote, or parse the source code from its
original source "Ziqian-Huang0607/deepgourou_supplychain_solution_chatbi", with no intent of the Prohibited Actions, 
while crediting the owner via a direct, non-misleading hyperlink, leading to the 
project repository. 

Attribution Protection
----------------------
You may not remove, replace, or modify the author’s name, copyright notice, 
or credits. Any attempt to re‑publish the Software under a different name 
or claim any kind of authorship is strictly prohibited.

Termination
-----------
Any unauthorized use of the Software immediately terminates any implied or 
previously granted permissions. Upon termination, you must remove all instances 
of the Software that you have infringed.

Governing Law
-------------
This license is governed by the laws of the jurisdiction in which the copyright 
holder resides, without regard to conflict‑of‑law principles.
